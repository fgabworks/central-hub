...
| **CLIMATE shell + Code Workspace v1** | Visible CLIMATE shell with `VANTA | ARCTIC` UI identities over compatible `work`/`personal` routes and compact scoped context labels; `/work/climate` + `/personal/climate`; read-only Monaco file viewer that fills the remaining editor area and scrolls long files; AI answers make `path/file.py — function` traces clickable into that viewer; Markdown Source/Preview, GFM chat rendering (marked + DOMPurify), filtered Explorer with Show excluded, search, multi-tabs, Git status/diff, Problems/Output/Tests/Git, persisted/resizable layout; provider-neutral Codex/Claude Code/Cursor Agent panel with exact model, cancel/output, bounded context, ASK logic-answer outline (prompt + conservative reorder), execution Mode (`CLIMATE Assisted` / `Direct Provider`) separate from Provider/Model, domain-aware resolver hints in Assisted only (simple reference lookup first; logs/bulk dumps downranked), `Explored N` = files actually read (including Get-Content), and review-gated proposal Accept/Reject (`hub/climate/`). File save/Git write are not in this viewer. |
