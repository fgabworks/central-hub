...
| **CLIMATE Branding** | `/settings/branding` — one PNG/SVG/WEBP file under `data/branding/` (path, not base64). **App Branding** = Wordmark / Full logo for sidebar, top-left header, and larger areas. **Chat Avatar** = icon-only (padded `object-fit: contain`) for AiriX message avatars and compact assistant identity; never the full wordmark lockup. Direct Gemini/Codex/Claude/Cursor keep provider icons; user stays the existing initial. Reset to default. |
