...
| **AiriX · CLIMATE Chat** | `/work/chat` + `/personal/chat`; ChatGPT-like Ask-only chat over existing Agent Center/Gemini conversations; compact `[ AiriX ] [ Direct ]` mode; Direct = provider general knowledge with optional explicit attachments; AiriX keeps CLIMATE evidence/orchestration; **No repository** is valid in both modes and not inherited from VANTA; no editor |
