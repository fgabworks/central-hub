...
| AI Assistant Center | Implemented — isolated Aira/AiriX profiles; full-height persistent dock + fixed composer; **AiriX Smart Routing Phase 5** (cost intelligence; RBAC Viewer/Analyst/Developer/Admin; relevance findings; budgets; multi-step orchestration; history/findings; Codex approval); coding CLIs require repository context (`repository_context.py`: explicit → persisted → workspace active → sole; never first-of-many); **dynamic scope + grounding** (`scope.py` / `data_intent.py` / `grounding.py` + `org_unit_lookup`) — structured data queries (counts/indicators/OU/period) → T0, never Hub Simulator; project/ambiguous+repo require Hub evidence; explicit national/GK/web overrides selected repo; T0 miss → cannot-verify for authoritative data, fall through for GK/national only otherwise; Source/Grounded status on results; **manual provider selection is authoritative** (no silent Hub Simulator fallback; unavailable → real error; Use Recommended required to accept Smart Routing); **AI usage telemetry** (`routing/telemetry.py`) on every execution — tier/type/LLM/provider/model/tokens/tools/runtime/child run; T0 forces zero AI tokens; dock unwraps AgentCenter `{run}` payloads and stops on terminal `completed|failed|cancelled|paused_for_approval|timed_out`; T0 deterministic auto-execute; Choose Agent one-shot manual override; Codex CLI real provider (read-only `codex exec --json`); provider switching; Find/Ask/Plan/Review; context preview; streaming/cancel/retry/history; scoped read-only tools; no write/execution; no voice |
