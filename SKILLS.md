...
| AI Assistant Center (read-only) | `/personal/aira`, `/work/airix`, dock on all pages via `templates/partials/assistant_dock_panel.html` + `static/js/assistant_dock.js`, `hub/agent_center/` (+ `routing/` Phase 3), `config/agents.yaml`, `data/agent_center.db` |
...
| AI Connections | `/system/ai-connections` — Installed/Authenticated/Version/Last Checked; connect uses visible provider login; Codex CLI first real provider (AiriX MVP) |
