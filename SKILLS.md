...
| AI Assistant Center | Implemented — isolated Aira/AiriX profiles; full-height persistent dock + fixed composer; **AiriX Smart Routing Phase 5** (cost intelligence; RBAC Viewer/Analyst/Developer/Admin; relevance findings; budgets; multi-step orchestration; history/findings; Codex approval); **Routing Mode** Smart vs Direct Agent — Efficient (persist per workspace; Direct skips T0/routing/escalation, exact provider+model, lightweight context only, session reuse, no silent fallback; security/grounding remain); coding CLIs require repository context (`repository_context.py`: explicit → persisted → workspace active → sole; never first-of-many); **dynamic scope + grounding + completion contract + capability escalation** (`scope.py` / `data_intent.py` / `grounding.py` / `completion.py` / `capability.py` + `org_unit_lookup`) — structured data → T0 then connected RO SQL before AI; Evidence Found ≠ Task Solved; Grounded=Yes only with authoritative evidence; escalate only when AI can help; preserve selected Codex model; project/ambiguous+repo require Hub evidence; explicit national/GK/web overrides selected repo; T0 miss → cannot-verify for authoritative data when no capable next step; **manual provider selection is authoritative** (no silent Hub Simulator fallback; unavailable → real error; Use Recommended required to accept Smart Routing); **AI usage telemetry** (`routing/telemetry.py`) — tier/type/LLM/provider/model/tokens/tools/runtime/child run + T0 failure/next capability/DB/AI escalate + routing mode/session/context; T0 forces zero AI tokens; dock unwraps AgentCenter `{run}` payloads and stops on terminal `completed|failed|cancelled|paused_for_approval|timed_out`; T0 deterministic auto-execute; Choose Agent one-shot manual override; Codex CLI real provider (read-only `codex exec --json`); provider switching; Find/Ask/Plan/Review; context preview; streaming/cancel/retry/history; scoped read-only tools; no write/execution; no voice |
