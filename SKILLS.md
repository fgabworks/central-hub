...
| **AiriX · CLIMATE Chat** | `/work/chat` + `/personal/chat`; compact `[ AiriX ] [ Direct ]` mode; Context Scope = General (default) / All Repositories (bounded relevant hits) / specific repository; Direct = provider general knowledge with optional attachments; AiriX keeps CLIMATE orchestration plus hub registry facts in General; lightweight thinking/streaming/cancel/error states; no editor |
| **CLIMATE shell + Code Workspace v1** | Visible CLIMATE shell with `VANTA | ARCTIC` UI identities over compatible `work`/`personal` routes and compact scoped context labels; `/work/climate` + `/personal/climate` **Workspace Assistant**; read-only Monaco file viewer that fills the remaining editor area and scrolls long files; **AiriX · CLIMATE Chat** identity over `[ AiriX / Direct ] [ Provider ] [ Model ] [ Context Scope ]` (General / All Repositories / specific repo) plus explicit file chips (Add to Chat, Clear All, `@filename`); server-backed conversation list/detail/rename via existing Agent Center SQLite conversations/runs with browser state as a rich local mirror; AI answers make `path/file.py — function` traces clickable into that viewer; Markdown Source/Preview, GFM chat rendering (marked + DOMPurify), filtered Explorer with Show excluded, search, multi-tabs, Git status/diff, Problems/Output/Tests/Git, persisted/resizable layout; provider-neutral Gemini/Codex/Claude Code/Cursor Agent panel with exact model, cancel/output, bounded context, ASK logic-answer outline (prompt + conservative reorder), execution Mode (`AiriX` / `Direct`) separate from Provider/Model, domain-aware resolver hints in Assisted only (simple reference lookup first; logs/bulk dumps downranked), `Explored N` = files actually read (including Get-Content), and review-gated proposal Accept/Reject (`hub/climate/`). Gemini is API-backed, dynamically discovers accessible models, streams responses, and is ASK-only with no editing, commands, tools, or native repo scan. File save/Git write are not in this viewer. |
...
| AI Connections | `/system/ai-connections` — CLIMATE provider cards + split Chat/Code Workspace defaults (`mode`/`provider`/`model`) and provider model overrides; local logos; API Key/CLI method badges; Test Connection / Manage; Gemini keys stored locally via Settings APIs (`data/ai_provider_secrets.env`, not encrypted, never returned); Codex / Claude Code / Cursor remain CLI-auth; compact panel also on Settings |
