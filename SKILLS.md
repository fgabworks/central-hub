...
| AI Connections | `/system/ai-connections` — CLIMATE provider cards + compact split Chat/Code Workspace defaults (`mode`/`provider`/`model`) and a one-row Provider Overrides grid; local logos; API Key/CLI method badges; Test Connection / Manage; Gemini/OpenAI/Anthropic/xAI keys stored locally via Settings APIs (`data/ai_provider_secrets.env`, not encrypted, never returned); Codex / Claude Code / Cursor remain CLI-auth; Cursor Agent resolves `agent` or `cursor-agent` from PATH (Windows User PATH + `%LOCALAPPDATA%\cursor-agent`) and Connected only after authenticated `agent status`; Grok is API-key/xAI only; compact panel also on Settings |
