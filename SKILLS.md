...
| AI Connections | `/system/ai-connections` — CLIMATE provider cards + split Chat/Code Workspace defaults and provider model overrides; local logos; API Key/CLI method badges; Test Connection / Manage; Gemini keys stored locally via Settings APIs (`data/ai_provider_secrets.env`, not encrypted, never returned); Codex / Claude Code / Cursor remain CLI-auth; compact panel also on Settings |
