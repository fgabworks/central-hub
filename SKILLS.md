...
| **CLIMATE shell + Code Workspace v1 + Coding Agent Phase 1** | Existing `/work/climate` + `/personal/climate` **AiriX · Code Assistant** IDE panel, read-only Monaco viewer, sessions, streaming/cancel, exact provider/model, AiriX/Direct, General/All/Specific scope, explicit file chips, Sources/Details, diagnostics, Git status/diff, and persisted layout. Specific Repository EDIT may use RepoBrain orientation plus live evidence and returns a persisted short plan, affected files, and bounded unified diff. Explicit Accept alone uses the guarded Repository Workspace save path; Reject and ordinary ASK write nothing. Raw-hash stale detection, path jail, vendor/generated/build/binary/secret exclusions, configurable patch limits, audit events, and rollback snapshots apply. Direct has the same approval gate and no autonomous writes. No arbitrary commands, tests, self-fixing, commit, or push. Gemini/OpenAI/Anthropic/xAI remain ASK-only. |
