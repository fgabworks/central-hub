...
| **CLIMATE shell + Code Workspace v1** | Visible CLIMATE shell with `VANTA | ARCTIC` UI identities over compatible `work`/`personal` routes and compact scoped context labels; `/work/climate` + `/personal/climate`; Monaco, filtered Explorer with Show excluded, search, safe open/save, multi-tabs, Git status/diff, Problems/Output/Tests/Git, persisted/resizable layout; provider-neutral Codex/Claude Code/Cursor Agent panel with exact model, cancel/output, bounded context, ASK logic-answer outline (prompt + conservative reorder), domain-aware resolver hints, `Explored N` = files actually read, and review-gated proposal Accept/Reject (`hub/climate/`) |
