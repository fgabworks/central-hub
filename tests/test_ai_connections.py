...
class AiConnectionsTests(unittest.TestCase):
    ...
    def test_system_page_and_navigation(self):
        ...
        self.assertIn("CLIMATE Chat (General)", html)
        self.assertIn("Code Workspace (Coding)", html)
        self.assertIn("Provider Overrides (Auto)", html)
        self.assertNotIn("aic-lede", html)
        self.assertNotIn("aic-field-wide", html)
        self.assertIn("aic-field-mode", html)
        self.assertIn("aic-field-provider", html)
        self.assertIn("aic-field-model", html)
        self.assertIn("aic-model-grid", html)
        ...
        css = (ROOT / "static" / "css" / "style.css").read_text(encoding="utf-8").replace("\r\n", "\n")
        aic_css = css[css.find("/* AI Connections"): css.find(".ai-provider-compact")]
        self.assertIn(".aic-page {\n  width: 100%;\n  max-width: none;", aic_css)
        self.assertIn(
            "grid-template-columns: minmax(11.5rem, 14.5rem) max-content minmax(11rem, 15rem) minmax(12rem, 16rem) minmax(0, 1fr);",
            aic_css,
        )
        self.assertIn("repeat(4, minmax(0, 13.5rem))", aic_css)
        self.assertIn("max-width: 16rem;", aic_css)
        self.assertIn("@media (max-width: 1180px)", aic_css)
        self.assertIn("@media (max-width: 820px)", aic_css)
        self.assertIn("@media (max-width: 720px)", aic_css)
