...
class SidebarShellContractTests(unittest.TestCase):
    def test_sidebar_collapse_and_dhis2_expand_markup(self):
        ...
        nav_src = (ROOT / "hub" / "notebook" / "workspace.py").read_text(encoding="utf-8")
        self.assertIn('id="hub-sidebar"', base)
        self.assertIn("sidebar-collapse-btn", base)
        self.assertIn("sidebar-scroll", base)
        self.assertIn("nav-group-toggle", base)
        self.assertIn("expandable", nav_src)
        self.assertIn('"expand_prefix": "dhis2"', nav_src)
        self.assertIn("is-sidebar-collapsed", css)
...
class ParamCardLayoutContractTests(unittest.TestCase):
    ...
    def test_two_row_layout_and_deferred_validation(self):
        ...
        self.assertIn("Generating report", js)
        self.assertIn("Generating selected area", js)
        self.assertIn("Generating breakdown", js)
...
class UiContractTests(unittest.TestCase):
    def test_result_type_aware_display_and_uid_drawer_hooks(self):
        ...
        nav_src = (ROOT / "hub" / "notebook" / "workspace.py").read_text(encoding="utf-8")
        # Tools grid (app.py) + Work sidebar (shared nav) — same endpoint, no duplicate route.
        self.assertEqual(app_src.count('"endpoint": "dhis2_hcsc_indicators"'), 1)
        self.assertIn("dhis2_hcsc_indicators", nav_src)
        self.assertIn("HCSC–RF", nav_src)
        # DHIS2 Reports precedes HCSC–RF inside the expandable DHIS2 group.
        reports_idx = nav_src.index("DHIS2 Reports")
        hcsc_nav_idx = nav_src.index("HCSC–RF", reports_idx)
