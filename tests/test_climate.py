...
class ClimateUiContractTests(unittest.TestCase):
    def test_mockup_shell_and_persistence_contract(self):
        ...
        self.assertIn("<summary>Details</summary>", script)
        self.assertNotIn("Details / Diagnostics", script)
        self.assertIn("renderTokenEfficiencyFold", script)
        self.assertIn("Completed · ", script)
        self.assertIn("Your code-aware AI partner", template)
        self.assertIn("climate-assistant-controls", template)
        self.assertIn("climate-assistant-footer", template)
        self.assertIn("climate-compose-stack", template)
        self.assertIn("climate-assistant-tagline", template)
        self.assertIn("@ to reference files, / for commands", template)
        header = template[template.find("climate-assistant-header"):template.find("climate-ai-feed")]
        composer = template[template.find('id="climate-chat-composer"'):]
        self.assertNotIn('id="climate-assistant-controls"', header)
        self.assertNotIn('id="climate-execution-mode"', header)
        self.assertNotIn('id="climate-assistant-context"', header)
        self.assertIn("climate-provider-state", header)
        self.assertIn("climate-chat-new", header)
        self.assertIn('id="climate-assistant-controls"', composer)
        self.assertIn('id="climate-execution-mode"', composer)
        self.assertIn('id="climate-provider-panel"', composer)
        self.assertIn('id="climate-model-panel"', composer)
        self.assertIn('id="climate-context-scope"', composer)
        self.assertLess(template.find("climate-ai-feed"), template.find("climate-assistant-footer"))
        self.assertLess(template.find("climate-assistant-context"), template.find("climate-chat-composer"))
        ...
        identity_fn = script.split("function identityLogoSrc", 1)[1].split("\n  function ", 1)[0]
        self.assertIn("data-brand-icon", identity_fn)
        self.assertIn("img/climate-mark.png", identity_fn)
        self.assertIn("img/providers/gemini.svg", identity_fn)
        self.assertIn("img/providers/codex.svg", identity_fn)
        self.assertIn("img/providers/claude-code.svg", identity_fn)
        self.assertIn("img/providers/cursor-agent.svg", identity_fn)
        self.assertNotIn("climate-logo.png", identity_fn)
        ...
        self.assertIn("persistSessionFromRun", script)
        self.assertIn("applyPersistedWorkspaceIdentity", script)
