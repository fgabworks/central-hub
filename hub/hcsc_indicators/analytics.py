...
def fetch_analytics_batch(
    ...
    *,
    ...
    try:
        ...
        chunk_size = int(breakdown_thresholds().get("analytics_ou_chunk") or 40)
    ...
    for i in range(0, len(ou_list), chunk_size):
        chunk = ou_list[i : i + chunk_size]
        ou_dim = ";".join(chunk)
        params = [
            ("dimension", f"dx:{';'.join(dx_uids)}"),
            ("dimension", f"pe:{period}"),
        ...
        if len(chunk) == 1 and not (parsed.get("by_ou") or {}):
            # Single-OU response without ou column — attribute to that OU.
            ou = chunk[0]
            merged_by_ou[ou] = {
                "values": dict(parsed.get("values") or {}),
                "num_den": dict(parsed.get("num_den") or {}),
            }
        else:
        ...
        last_request_meta = analytics_request_description(
            dx_uids=dx_uids,
            period=period,
            org_unit=chunk if len(chunk) > 1 else chunk[0],
            include_num_den=include_num_den,
        )
