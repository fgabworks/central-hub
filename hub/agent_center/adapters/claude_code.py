...
class ClaudeCodeAdapter(BaseCliAdapter):
    authentication_method = "Claude Code browser authentication"

    def _authentication_probe(self, executable: str) -> dict[str, Any]:
        result = self._run_probe([executable, "auth", "status"])
        raw = (result.stdout or result.stderr or "").strip()
        if result.returncode != 0:
            return {"state": "authentication_required", "detail": "Claude Code authentication required"}
        account = ""
        try:
            ...
            pass
        return {"state": "connected", "detail": "Claude Code authenticated", "account_label": account}
    ...
    def list_model_details(self, *, mode: str = "ask", force_refresh: bool = False) -> dict[str, Any]:
        ...
        rows = [{"id": "__provider_default__", "display_name": "Provider default", "availability": "available"}] if models else []
        return {"models": models, "model_details": rows, "groups": {}, "recommended_model": models[0] if models else None, "models_source": source, "reasoning_efforts": [], "error": ""}
